...
class AndroidDevice(DeviceInterface):
    ...
    def _set_device_dependencies(self):
        ...
        if self.device_info and not self.device_info.identifier:
            self._logger.warning("Device identifier was not specified. Note the device specified "
    ...
    def execute(self, commands: List[str],
        ...
        if isinstance(device_return, DeviceCompletedProcess):
            self._logger.info(f"Executed command: {android_shell_commands} with return code: "
                              f"{device_return.returncode}")
    ...
    def get_device_log(self, date_range: Optional[DateRange] = None) -> bytes | None:
        """
        Get the entire device log or the log within a specified time range.

        Args:
            date_range (Optional[DateRange]): The date range of the device log.
                                              Defaults to None.
        ...
        adb_stdout = device_return.stdout if isinstance(device_return.stdout, bytes) \
                     else bytes(device_return.stdout, encoding="utf-8")

        if date_range:
            adb_stdout = get_log_in_date_range(adb_stdout, date_range)
            if not adb_stdout:
                self._logger.debug(f"No log was found in date range: {date_range!s}")
    ...
    def get_available_devices(connection_type: ConnectionType = ConnectionType.LOCAL,
        ...
        Returns:
            Optional[List[DeviceInfo]]: The available devices.
        """
        return AndroidDevice._get_available_devices_for_platform(connection_type, hostname, ip_addr, port,
