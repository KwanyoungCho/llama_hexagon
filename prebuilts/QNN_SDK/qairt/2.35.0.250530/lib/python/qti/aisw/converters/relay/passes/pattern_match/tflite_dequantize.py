...
class DequantizeQnnConv2dTransposePattern(DequantizeQnnPattern):
    ...
    def get_pattern(self):
        ...
        _bias_add = _conv2d_transpose.optional(lambda x: is_op('nn.bias_add')(x, is_constant()))
    ...
    def dequantize_qnn_expr(self, requantize_expr, args):
        ...
        if conv2d_transpose_expr.attrs['kernel_layout'] == 'IOHW' and kernel_scale.size!=1:
            kernel_scale = np.expand_dims(kernel_scale, axis=1)
        ...
        if args[0].op.name == 'nn.bias_add':
            ...
            new_bias_expr = self.dequantize_constant_expr(bias_expr, bias_scale, bias_zero_point)
            new_conv2d_transpose_expr = tvm.relay.nn.bias_add(new_conv2d_transpose_expr, new_bias_expr, **bias_add_expr.attrs)

        new_output_expr = self.dequantize_output_expr(new_conv2d_transpose_expr, requantize_expr.span, output_scale, output_zero_point, requantize_expr.attrs['out_dtype'])
