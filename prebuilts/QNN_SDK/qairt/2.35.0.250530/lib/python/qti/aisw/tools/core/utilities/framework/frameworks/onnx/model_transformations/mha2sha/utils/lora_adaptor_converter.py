...
from .encoding_mapper_utils import (
    ...
    update_sha_enc_name_for_v1_ptq,
)
...
def parse_lora_adaptor_list(lora_adaptor_list_path):
    ...
    with open(lora_adaptor_list_path, "r") as file:
        lora_adaptor_list = yaml.safe_load(file)["use_case"]
...
class LoraAdaptor:
    ...
    def __init__(self, lora_adaptor_dict) -> None:
        ...
        self.safetensor_path = lora_adaptor_dict["lora_weights"]
        self.encodings_path = lora_adaptor_dict["quant_overrides"]
        self._safetensor = None
        self._encodings = None
        assert os.path.exists(
            self.safetensor_path
        ), f"Lora adaptor safetensor path {self.safetensor_path} not exist."
        assert os.path.exists(
            self.encodings_path
        ), f"Lora adaptor encoding path {self.encodings_path} not exist."
    ...
    def _check_adaptor_weights(self, lora_adaptor_sha_tensor_dict):
        ...
        if sha_adaptor_diff := (LoraAdaptor.base_sha_lora_keys - lora_adaptor_keys):
            log_warning(
                f"{list(sha_adaptor_diff)}: exits in sha_weight.safetensor but not found in {self.name}_{LORA_SHA_SAFETENSOR_PATH}."
            )
    ...
    def _check_adaptor_encodings(self, lora_adaptor_sha_encodings, base_sha_encoding):
        ...
        for activ_param_encoding in ("activation_encodings", "param_encodings"):
            key_diff = set(base_sha_encoding[activ_param_encoding].keys()) ^ set(
                lora_adaptor_sha_encodings[activ_param_encoding].keys()
            )
            assert not key_diff, f"{activ_param_encoding} base model sha and lora adpator sha encoding keys do not match. Got {key_diff=}"
            ...
            for tensor in base_sha_encoding[activ_param_encoding].keys():
                ...
                if base_model_sha_channel != base_model_sha_channel:
                    if activ_param_encoding == "param_encodings" and (
                        adaptor_sha_channel == 1 or base_model_sha_channel == 1
                    ):
                        # log warning because it can be base model is in PCQ but lora adaptor in PTQ.
                        log_warning(
                            f"[{activ_param_encoding}][{tensor}]: base model sha and lora adpator sha encoding len not match, got {base_model_sha_channel} and {adaptor_sha_channel}"
                        )
                    else:
                        log_assert(
                            f"[{activ_param_encoding}][{tensor}]: base model sha and lora adpator sha encoding len not match, got {base_model_sha_channel} and {adaptor_sha_channel}"
    ...
    def map_encoding_and_slice_safetensor(self):
        ...
        for lora_adaptor_weight_name, lora_adaptor_tensor in self.safetensor.items():
            ...
            if lora_adaptor_weight_name in LoraAdaptor.mha_to_sha_encodings_names["param_encodings"]:
                sha_weight_name_list = LoraAdaptor.mha_to_sha_encodings_names["param_encodings"][
                    lora_adaptor_weight_name
                ]
                ...
                for head, sha_weight_names in enumerate(sha_weight_name_list):
                    if LoraAdaptor.mha_conv:
                        lora_adaptor_sha_tensor_dict[sha_weight_names] = lora_adaptor_tensor[
                            head_dim * head : head_dim * (head + 1), ...
                        ]  # [O, I, Kh, Kw]
                    else:
                        lora_adaptor_sha_tensor_dict[sha_weight_names] = lora_adaptor_tensor[
                            :, head_dim * head : head_dim * (head + 1)
                        ]  # [I, O]
            # Preserve the remaining weights as-is
            else:
                lora_adaptor_sha_tensor_dict[lora_adaptor_weight_name] = lora_adaptor_tensor

        if excluded_lora_adaptor_names:
            log_warning(
                f"{excluded_lora_adaptor_names}:weights from {self.safetensor_path} not found in {LORA_SHA_SAFETENSOR_PATH}"
            )
        ...
        for activ_param_encoding in ("activation_encodings", "param_encodings"):
            for lora_adaptor_mha_name, lora_adaptor_mha_encoding in mha_encodings[
                activ_param_encoding
            ].items():
                # Update tensor encodings in mha
                if lora_adaptor_mha_name in LoraAdaptor.mha_to_sha_encodings_names[activ_param_encoding]:
                    sha_encoding_name_list = LoraAdaptor.mha_to_sha_encodings_names[activ_param_encoding][
                        lora_adaptor_mha_name
                    ]
                    head_num = len(sha_encoding_name_list)
                    head_dim = len(lora_adaptor_mha_encoding) // head_num
                    ptq_or_activation = (
                        AimetEncodingV1Types(lora_adaptor_mha_encoding["enc_type"])
                        == AimetEncodingV1Types.PER_TENSOR
                        if is_v1_aimet_encodings
                    ...
                    if activ_param_encoding == "param_encodings" and ptq_or_activation:
                        log_warning(
                            f"Got PTQ param encoding for LoRA weight {lora_adaptor_mha_name}, applying same param encoding to all these sha weights: {sha_encoding_name_list}."
                        )
                        ...
                        if is_v1_aimet_encodings:
                            if activ_param_encoding == "param_encodings":
                                if ptq_or_activation:
                                    sliced_mha_encoding = update_sha_enc_name_for_v1_ptq(
                                        lora_adaptor_mha_encoding, sha_name
                                    )
                                else:
                                    sliced_mha_encoding = handle_aimet_v1_param_encodings(
                                        lora_adaptor_mha_encoding, sha_name, head, head_num
                                    )
                            else:
                                sliced_mha_encoding = handle_aimet_v1_activ_encodings(
                                    lora_adaptor_mha_encoding, sha_name
                                )

                        else:
                            sliced_mha_encoding = (
                                lora_adaptor_mha_encoding[head * head_dim : (head + 1) * head_dim]
                                if not ptq_or_activation
                                else lora_adaptor_mha_encoding
                            )
        ...
        log_debug(
            f"{not_updated_encodings_list}: Encodings from {self.encodings_path} are not lora-related encodings. Not update."
        )
