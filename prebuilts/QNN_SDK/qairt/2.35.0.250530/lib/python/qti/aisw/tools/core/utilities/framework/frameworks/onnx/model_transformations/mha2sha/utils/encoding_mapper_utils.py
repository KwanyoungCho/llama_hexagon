...
def update_sha_tensor_to_node_mapping_dict(node_mapping_dict, sha_node_list):
    ...
    if "sha_input_1_activation_name" in node_mapping_dict.sha_mapping_name_list:
        node_mapping_dict.mapping_name_dict["sha_input_1_activation_name"] = [
            node.input[0] for node in sha_node_list
        ]

    if "sha_input_2_activation_name" in node_mapping_dict.sha_mapping_name_list:
        node_mapping_dict.mapping_name_dict["sha_input_2_activation_name"] = [
            node.input[1] for node in sha_node_list
        ]

    if "sha_output_activation_name" in node_mapping_dict.sha_mapping_name_list:
        node_mapping_dict.mapping_name_dict["sha_output_activation_name"] = [
            node.output[0] for node in sha_node_list
        ]
...
def update_multi_input_concat_sha_tensor_to_node_mapping_dict(node_mapping_dict, sha_node):
    ...
    if "sha_input_1_activation_name" in node_mapping_dict.sha_mapping_name_list:
        node_mapping_dict.mapping_name_dict["sha_input_1_activation_name"] = [
            _input for _input in sha_node.input
        ]
...
def copy_mha_activation_encodings_to_sha(
    ...
    for _mha_acitvation_name, _sha_acitvation_name in zip(mha_mapping_name_list, sha_mapping_name_list):
        ...
        if mha_encoding_key not in _mha_to_sha_encodings_names_to_dump["activation_encodings"]:
            _mha_to_sha_encodings_names_to_dump["activation_encodings"][mha_encoding_key] = (
                sha_encoding_key_list
            )
        else:
            for _sha_enc_key in sha_encoding_key_list:
                if (
                    _sha_enc_key
                    not in _mha_to_sha_encodings_names_to_dump["activation_encodings"][mha_encoding_key]
                ):
                    _mha_to_sha_encodings_names_to_dump["activation_encodings"][mha_encoding_key].extend(
                        sha_encoding_key_list
                    )
        ...
        if mha_encoding_key in mha_activation_encoding.keys():
            ...
            if handle_potential_concat and _sha_acitvation_name == "sha_output_activation_name":
                ...
                assert (
                    head_num == len(sha_encoding_key_list)
                ), f"sha encoding for mha node: {qkv_node_name}, {mha_encoding_key}, sha activation key num: {len(sha_encoding_key_list)} is different to head_num: {head_num} "
            ...
            try:
                ...
                if not any([_sha_key == mha_encoding_key for _sha_key in sha_encoding_key_list]) and (
                    mha_encoding_key not in model_input_output_list
                ):
                ...
                assert mha_encoding_key in [
                    node_mapping_dict.mapping_name_dict[_mha_acitvation_name]
                    for _mha_acitvation_name in mha_mapping_name_list
                ]
        else:
            log_debug(
                f"mha activation encoding for attention within {start_node_name}, node: {qkv_node_name} key: {mha_encoding_key} not exist in mha encoding file"
            )
...
def update_sha_enc_name_for_v1_ptq(encoding, sha_key):
    sha_encoding = copy.deepcopy(encoding)
    sha_encoding["name"] = sha_key
    return sha_encoding


def copy_mha_param_encodings_to_sha(
    mha_param_encoding,
    sha_param_encoding,
    _start_node,
    qkv_node_name,
    ...
    for _mha_param_name, _sha_param_name in zip(mha_mapping_name_list, sha_mapping_name_list):
        ...
        if mha_encoding_key in mha_param_encoding.keys():
            ...
            if is_ptq_encoding:
                log_warning(
                    f"Got PTQ param encoding on {mha_encoding_key}, applying same param encoding to all these sha weights: {sha_encoding_key_list}."
                )

            assert head_num == len(
                sha_encoding_key_list
            ), f"sha param key num: {len(sha_encoding_key_list)} is different to head_num {head_num} "
            for i, _sha_key in enumerate(sha_encoding_key_list):
                if is_v1_aimet_encodings:
                    if is_ptq_encoding:
                        sha_param_encoding[_sha_key] = update_sha_enc_name_for_v1_ptq(encoding, _sha_key)
                    else:
                        sha_param_encoding[_sha_key] = handle_aimet_v1_param_encodings(
                            encoding, _sha_key, i, head_num
                        )
                else:  # aimet v0.6.1 encodings
                    slice_len = len(encoding) // head_num
                    sha_param_encoding[_sha_key] = (
                        encoding[get_encoding_slice(i, slice_len)] if not is_ptq_encoding else encoding
                    )
            ...
            log_warning(
                f"mha parameter encoding for attention within {_start_node}, node: {qkv_node_name} key: {mha_encoding_key} not exist in mha encoding file"
            )
...
def create_tensor_name_to_encodings(encodings):
    ...
    return {
        f"{attr}_encodings": {_encoding["name"]: _encoding for _encoding in encodings[f"{attr}_encodings"]}
