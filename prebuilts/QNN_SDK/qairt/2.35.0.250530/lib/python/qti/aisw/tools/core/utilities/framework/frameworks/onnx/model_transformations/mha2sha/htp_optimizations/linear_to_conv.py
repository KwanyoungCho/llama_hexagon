...
class LinearToConv:
    ...
    def replace(self) -> None:
        ...
        for linear_node in tqdm(linear_nodes):
            ...
            has_model_output_in_linear_output = any(
                [
                    output in self.mha2sha_optim.mha_model_output_names
                    for output in linear_node.output
                ]
            )
            ...
            last_transpose = self._get_htp_conv_from_linear(
                ...
                last_transpose_output=linear_info["output_name"]
                if has_model_output_in_linear_output
                else None,
            )
    ...
    def _get_linear_info(self, linear_node: NodeProto, is_gemm: bool) -> LinearInfo:
        ...
        def get_weight_or_bias_info(get_weight: bool = True) -> LinearTensorInfo:
            ...
            WEIGHT, BIAS = 1, 2
            linear_weight_or_bias_name = linear_node.input[
                WEIGHT if get_weight else BIAS
            ]
            return {
                "name": linear_weight_or_bias_name,
                "numpy": numpy_helper.to_array(
                    self.mha2sha_optim.get_initializer_by_name[
                        linear_weight_or_bias_name
                    ]
                ),
                "idx_in_initializer": self.mha2sha_optim.get_initializer_idx_by_name[
                    linear_weight_or_bias_name
                ],
            }
        ...
        linear_info = {
            ...
            "input_node": self.mha2sha_optim.get_node_by_output_name[
                linear_node.input[0]
            ],
            ...
            linear_info.update(
                {
                    ...
                    "attributes": {
                        attr.name: attr.f
                        for attr in linear_node.attribute  # .f is the float value of the attribute
                    },
                }
            )

            assert (
                linear_info["attributes"]["alpha"] == 1
            ), "Currently, only Gemms with alpha equal to 1 are supported"
            assert (
                linear_info["attributes"]["beta"] == 1
            ), "Currently, only Gemms with beta equal to 1 are supported"

        return linear_info

    def _map_transpose_outputs_to_linears(
        self, linear_info: LinearInfo, last_transpose: NodeProto
    ) -> None:
        ...
        output_nodes_of_linear = list(
            chain.from_iterable(
                map(
                    lambda o: self.mha2sha_optim.get_node_by_input_name[o],
                    linear_info["output"],
                )
            )
        )
        ...
        for output_node in output_nodes_of_linear:
            index_of_linear_output = [
                idx
                for idx, inp in enumerate(output_node.input)
                if inp == linear_info["output_name"]
            ][0]
    ...
    def _get_htp_conv_from_linear(
        ...
        init_list = []
        node_list = []
        linear_weight_should_transpose = not is_gemm or (
            is_gemm and "transA" in linear_info["attributes"]
        )
        ...
        conv_as_linear_node = self.mha2sha_optim._op_factory.get_conv_op(
            ...
            output_tensor_name=linear_info["output_name"]
            if linear_info["output_name"] != last_transpose_output
            else None,
        )
        ...
        if squeeze_output:
            squeeze_output_dim, squeeze_output_dim_list = (
                self.mha2sha_optim._op_factory.get_squeeze_op(
    ...
    def _get_conv_weight_bias_init(
        ...
        if is_gemm:
            conv_bias_init = numpy_helper.from_array(
                linear_info["bias"]["numpy"], name=linear_info["bias"]["name"]
            )

            weight_bias_to_idx.append(
                (conv_bias_init, linear_info["bias"]["idx_in_initializer"])
            )
    ...
    def _handle_input_and_reshape_into_conv(
        ...
        vector_inp_dim = linear_info["weight"]["numpy"].shape[
            0 if linear_weight_should_transpose else 1
        ]
        if is_gemm and linear_info["attributes"].get("transB", None) == 1:
            ...
            reshape_node, reshape_init = self.mha2sha_optim._op_factory.get_reshape_op(
                input_node, [1, 1, vector_inp_dim, -1]
            )
            # [1, 1, vector_inp_dim, seq_len] -> [1, vector_inp_dim, seq_len, 1], B = 1
            transpose_to_BCHW = self.mha2sha_optim._op_factory.get_transpose_op(
                reshape_node, [0, 2, 3, 1]
            )
        else:
            reshape_node, reshape_init = self.mha2sha_optim._op_factory.get_reshape_op(
                input_node, [1, 1, -1, vector_inp_dim]
            )
            # [1, 1, seq_len, vector_inp_dim] -> [1, vector_inp_dim, seq_len, 1], B = 1
            transpose_to_BCHW = self.mha2sha_optim._op_factory.get_transpose_op(
                reshape_node, [0, 3, 2, 1]
            )
    ...
    def _handle_last_transpose_and_squeeze(
        ...
        if last_transpose_output is not None:
            for model_output in self.mha2sha_optim.model.graph.output:
                if model_output.name == last_transpose_output:
                    output_dim = len(
                        [d.dim_value for d in model_output.type.tensor_type.shape.dim]
                    )
                    assert output_dim in (
                        3,
                        4,
                    ), f"expect model output dim = 3 or 4, but got {output_dim}"
                    if output_dim == 3:
                        squeeze_output = True
                    break
        ...
        return self.mha2sha_optim._op_factory.get_transpose_op(
            ...
            output=last_transpose_output
            if (input_dim != 3 and not squeeze_output)
            else None,  # Only special output if model output and Transpose is last
        ), squeeze_output
