...
def load_gguf_checkpoint(gguf_checkpoint_path: object, return_tensors: object = False,
                         extract_encodings: object = False, preserve_original_names: object = False) -> object:
    """
    ...
    Args:
        ...
        extract_encodings ('bool', defaults to 'False'):
            Whether to return scale and offset along with de-quantized tensor.
        preserve_original_names('bool', defaults to 'False'):
            Whether to preserve original tensor names.
    """
    try:
    ...
    if return_tensors:
        ...
        for tensor in tqdm(reader.tensors, desc="Converting and de-quantizing GGUF tensors..."):
            ...
            shape = tensor.shape
            name = tensor.name
            weights, encodings = load_dequant_gguf_tensor(shape=shape, ggml_type=tensor.tensor_type,
            ...
            if architecture == "llama" and (".attn_k." in name or ".attn_q." in name):
                ...
                if ".attn_q." in name:
                    ...
                    if encodings.get("scale", None):
                        encodings = reverse_permute_encodings(encodings, weights.shape, num_heads, num_heads)
                elif ".attn_k." in name:
                    weights = reverse_permute_weights(weights, num_heads, num_kv_heads)
                    if encodings.get("scale", None):
                        encodings = reverse_permute_encodings(encodings, weights.shape, num_heads, num_kv_heads)
            ...
            parsed_parameters["tensors"][name] = torch.from_numpy(np.copy(weights)) if not extract_encodings else np.copy(weights)

            # store encodings
            if extract_encodings:
                encodings['name'] = name
                parsed_parameters["encodings_map"][name] = encodings
...
def reverse_permute_encodings(encodings: dict, shape: np.ndarray, n_head: int, num_kv_heads: Optional[int] = None) -> dict:
    # permutation implementation
    if num_kv_heads is not None and n_head != num_kv_heads:
        n_head = num_kv_heads
    encoding_scale = np.array(encodings['scale'])
    encoding_scale_shape = encoding_scale.shape[0]
    encoding_scale = encoding_scale.reshape(shape[0], encoding_scale_shape // shape[0])
    dim = encoding_scale.shape[0] // n_head // 2
    w = encoding_scale.reshape(n_head, dim, 2, *encoding_scale.shape[1:])
    encodings['scale'] = w.swapaxes(2, 1).reshape(encoding_scale.shape).flatten().tolist()
    return encodings
