    ...
    def transform_layer(self, ir_graph, converter_context, descriptor, input_descriptors, output_descriptors):
        ...
        if descriptor.is_act_quant:
            ...
            if isinstance(descriptor, FakeQuantPerChannelResolver.Descriptor):
                raise ConverterError("Per-channel quantization of activation tensors is unsupported")
            ...
            for output_descriptor in output_descriptors:
                ...
                if isinstance(descriptor, FakeQuantPerChannelResolver.Descriptor):
                    # per-channel quantization requires signed symmetric schema,
                    # so has to provide signed symmetric encodings here,
                    # see function IrQuantizer::quantizePerChannel() for more details.
                    max_encoding = np.maximum(np.abs(descriptor.min), np.abs(descriptor.max))
                    min_encoding = -max_encoding
                    ir_graph.add_quantization_params(output_descriptor.layer_name,
                                                 ...
                                                 param_encodings={"name": "weights",
                                                                  ...
                                                                  "is_symmetric":descriptor.is_symmetric,
                                                                  "overridden": True})
                else:
                    ir_graph.add_quantization_params(output_descriptor.layer_name,
                                                    # currently only weights are quantized in TF so using that as name
