...
class MhaMapper:
    ...
    def _verify_mha_and_get_qkv_attn(self) -> Union[Tuple[List[NodeProto], List[NodeProto]], bool]:
        ...
        if (
            qk_matmul_node.input[0] not in self.get_node_by_output_name
            or qk_matmul_node.input[1] not in self.get_node_by_output_name
            or qkv_matmul_node.input[0] not in self.get_node_by_output_name
            or qkv_matmul_node.input[1] not in self.get_node_by_output_name
        ):
            return False
        ...
        if self.mha_conv:
            ...
            if len(qkv) == 3:
                ...
                if (
                    q_weight.shape[1] == embed_dim
                    and k_weight.shape[1] == embed_dim
                    and v_weight.shape[1] == embed_dim
                ):
                    return (qkv, attn)
    ...
    def verify_and_export_mha_info(self) -> Union[MhaInfo, bool]:
        ...
        if result:
            mha_info = MhaInfo(
                first_module_name=self.mha_potential[0].name,
                qkv_proj=result[0],
                attn_proj=result[1],
...
class AttentionFinder:
    ...
    def get_attn_info(self):
        ...
        if self.mha_conv:
            ...
            hard_stop_module_type_set = (
                "LayerNorm",
                "InstanceNormalization",
                "ReduceMean",
            )  # Hard stop for collecting potential mha pattern
