...
def generate_float_encodings(bw):
    encodings = dict()
    encodings['bw'] = bw
    encodings['dtype'] = 'FLOAT'
    return encodings


def update_negative_scales(values, scales):
    scaled_max_values = np.max(np.abs(values), axis=1, keepdims=True) / 7
...
def dequantize_q4_k(data):
    ...
    encodings = generate_encodings(32, 4, 'INT', 'PER_BLOCK', False, offsets, factors)
...
def dequantize_q4_0(data):
    ...
    scales = update_negative_scales(values, scales)
    offsets = np.ones(scales.shape, dtype=int) * (-8)
    encodings = generate_encodings(32, 4, 'INT', 'PER_BLOCK', True, offsets, scales)
    return values, encodings
...
def dequantize_q6_k(data):
    ...
    encodings = generate_float_encodings(FLOAT_BW)
    return values, encodings
...
def dequantize_q8_0(data):
    ...
    values = scales * qs
    encodings = generate_encodings(32, 8, 'INT', 'PER_BLOCK', True, np.array([]), scales)
    return values, encodings
...
def dequantize_q2_k(data):
    ...
    encodings = generate_encodings(16, 2, 'INT', 'PER_BLOCK', False, offsets, scales)
    return values, encodings
...
def dequantize_q3_k(data):
    ...
    encodings = generate_encodings(16, 3, 'INT', 'PER_BLOCK', True, np.array([]), scale_factors)
    return values, encodings
