...
from qti.aisw.accuracy_debugger.lib.utils.snooper_utils import show_progress, LayerStatus, files_to_compare, append_to_intermediate_report
...
class LayerwiseSnooping(Snooper):
    ...
    def run(self, model=None, profile_info=None):
        ...
        layerwise_intermediate_report_path = self.output_dir + '/intermediate_layerwise.csv'
        ...
        while (True):
            ...
            if prev_layer_out_name is not None:
                ...
                try:
                    self.logger.info('Extraction Starting Node:{} and End Node:{}'.format(
                ...
                if not skip_compare:
                    ...
                    if tensors_info is not None and sanitized_cur_layer_out_name in tensors_info:
                        inf_raw, is_permuted = permute_tensor_data_axis_order(
                            inf_raw, tensors_info[sanitized_cur_layer_out_name])
        ...
        for verifier_name in overall_comp_list:
            try:
                ...
                Visualizers.line_plot(x=df.index.values, y=df[verifier_name].values,
