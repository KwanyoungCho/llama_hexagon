...
from qti.aisw.tools.core.utilities.framework.frameworks.onnx.model_transformations.mha2sha.utils.op_factory import (
    create_tensor_name,
)
...
class MhaConvExtension:
    ...
    def get_conv_bias_in_OI(self, conv_node):
        if len(conv_node.input) <= 2:
            return None
        conv_bias_init = numpy_helper.to_array(self.mha2sha_optim.get_initializer_by_name[conv_node.input[2]])
    ...
    def get_qkv_info(
        ...
        if not (query_conv and key_conv and value_conv):
            ...
            log_debug(f"QKV MatMuls : {query_conv.name}, {key_conv.name}, {value_conv.name}")
    ...
    def create_single_conv(
        ...
        assert sha_weight_name == conv_init_name, "sha_query_weight_name and created init name are differnet"
        ...
        conv_init = numpy_helper.from_array(
            qkv_init[:, head_num * head_dim : (head_num + 1) * head_dim].T[..., None, None],
            conv_init_name,
        )
    ...
    def create_sha_qkv_convs(
        ...
        value_conv = None
        if (
            init_name in info_dict["query"].keys()
            and (conv_weight_init := info_dict["query"][init_name]) is not None
        ):
        ...
        if (
            init_name in info_dict["key"].keys()
            and (conv_weight_init := info_dict["key"][init_name]) is not None
        ):
        ...
        if (
            init_name in info_dict["value"].keys()
            and (conv_weight_init := info_dict["value"][init_name]) is not None
        ):
    ...
    def create_sha_conv_with_rope(
        ...
        if not self.mha2sha_optim.nchw_aligned:
            ...
            if query_matmul_inp == key_matmul_inp == value_matmul_inp:
                shared_NCHW_transpose_inp = self.mha2sha_optim._op_factory.get_transpose_op(
                    query_matmul_inp, [0, 3, 1, 2]
                )  # [N, H, W, C] to [N, C, H, W]
                ...
                query_matmul_inp = self.mha2sha_optim._op_factory.get_transpose_op(
                    query_matmul_inp, [0, 3, 1, 2]
                )  # [N, H, W, C] to [N, C, H, W]
                key_matmul_inp = self.mha2sha_optim._op_factory.get_transpose_op(
                    key_matmul_inp, [0, 3, 1, 2]
                )  # [N, H, W, C] to [N, C, H, W]
                value_matmul_inp = self.mha2sha_optim._op_factory.get_transpose_op(
                    value_matmul_inp, [0, 3, 1, 2]
                )  # [N, H, W, C] to [N, C, H, W]
                self.mha2sha_optim.model.graph.node.extend(
                    [query_matmul_inp, key_matmul_inp, value_matmul_inp]
                )
        ...
        if self.mha2sha_optim.gqa_model:
            conv_weight_init = info_dict["query"].get("matmul_init")
            if conv_weight_init is not None:
            ...
            if self.mha2sha_optim.lora_model:
                query_inp = self.mha2sha_optim._lora_extension.attach_single_lora_adaptor(
                    ...
                    query_inp,
                    BranchType.Q,
                    sha_base_attn_node_list.lora_q,
                )
            ...
            query_inp = self.mha2sha_optim._rope_extension.create_llama_rope_node(
            ...
            key_transpose = self.mha2sha_optim._gqa_extension.key_groups_list[
                int(head_num // head_num_per_group)
            ]
            value_inp = self.mha2sha_optim._gqa_extension.value_groups_list[
                int(head_num // head_num_per_group)
            ]
            ...
            # Step 2: Add lora adaptor (Optional)
            if self.mha2sha_optim.lora_model:
                query_inp, key_inp, value_inp = self.mha2sha_optim._lora_extension.attach_lora_adapters(
                    info_dict, ns, head_num, head_dim, sha_base_attn_node_list, query_inp, key_inp, value_inp
                )
            ...
            key_inp = self.mha2sha_optim._rope_extension.create_llama_rope_node(
            ...
            key_transpose = self.mha2sha_optim._op_factory.get_transpose_op(
                key_inp, [0, 1, 3, 2], propose_sha_key_name + "_Transpose"
            )
            ...
            value_transpose = self.mha2sha_optim._op_factory.get_transpose_op(
                value_inp, [0, 2, 3, 1], propose_sha_value_name + "_Transpose"
            )
            ...
            if self.mha2sha_optim.handle_past_key_value and self.mha2sha_optim.kv_cache:
                ...
                key_transpose, value_inp = self.mha2sha_optim._past_kv_extension.concat_past_key_value_input(
        ...
        # value_inp: [B, 1, L, D]
        qk_matmul_node, _ = self.mha2sha_optim._op_factory.get_matmul_op(
        ...
        qkv_matmul = self.mha2sha_optim.create_sha_pattern(
            info_dict,
            ns,
            head_num,
            head_dim,
    ...
    def create_sha_conv(
        ...
        if self.mha2sha_optim._scale_on_q_branch:
            query_inp = self.mha2sha_optim.add_scale_on_qk_branch(
                info_dict,
                query_inp,
                self.mha2sha_optim._scale_on_q_branch,
                propose_sha_name,
                sha_base_attn_node_list,
            )
        ...
        for tensor in self.mha2sha_optim.model.graph.value_info:
            if tensor.name == query_matmul_inp.output[0]:
                query_inp_shape = [dim.dim_value for dim in tensor.type.tensor_type.shape.dim]
            if tensor.name == key_matmul_inp.output[0]:
                key_inp_shape = [dim.dim_value for dim in tensor.type.tensor_type.shape.dim]
            if tensor.name == value_matmul_inp.output[0]:
                value_inp_shape = [dim.dim_value for dim in tensor.type.tensor_type.shape.dim]

        if query_inp_shape[0] != info_dict["batch"]:
            batch_idx = query_inp_shape.index(info_dict["batch"])
            head_idx = query_inp_shape.index(1)
            permute_order = [batch_idx, head_idx, 1]
            for idx in range(len(query_inp_shape)):
                if idx not in permute_order:
                    seq_len_idx = idx
            permute_order = [batch_idx, head_idx, seq_len_idx, 1]
            query_inp = self.mha2sha_optim._op_factory.get_transpose_op(query_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(query_inp)

        elif query_inp_shape[-1] == 1 and query_inp_shape[-2] == 1:
            permute_order = [
                2,
                3,
                0,
                1,
            ]  # (seq_len, head_dim, 1, 1) -> (1, 1, seq_len, head_dim)
            query_inp = self.mha2sha_optim._op_factory.get_transpose_op(query_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(query_inp)
            head_idx = 3
        elif query_inp_shape[-1] == 1:
            permute_order = [0, 3, 2, 1]  # (B, head_dim, H, 1) -> (B, 1, H, head_dim)
            query_inp = self.mha2sha_optim._op_factory.get_transpose_op(query_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(query_inp)
            head_idx = 3
        elif query_inp_shape[-2] == 1:
            permute_order = [0, 2, 3, 1]  # (B, head_dim, 1, W) -> (B, 1, W, head_dim)
            query_inp = self.mha2sha_optim._op_factory.get_transpose_op(query_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(query_inp)
            head_idx = 2
        else:
        ...
        if query_inp_shape[head_idx] != 1:
            permute_order = [0, 2, 3, 1]  # (B, head_dim, H, W) -> (B, H, W, head_dim)
            query_inp = self.mha2sha_optim._op_factory.get_transpose_op(query_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(query_inp)

        if key_inp_shape[0] != info_dict["batch"]:
            batch_idx = key_inp_shape.index(info_dict["batch"])
            head_idx = key_inp_shape.index(1)
            permute_order = [batch_idx, head_idx, 1]
            for idx in range(len(key_inp_shape)):
                if idx not in permute_order:
                    seq_len_idx = idx
            permute_order = [batch_idx, head_idx, 1, seq_len_idx]
            key_inp = self.mha2sha_optim._op_factory.get_transpose_op(key_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(key_inp)

        elif key_inp_shape[-1] == 1 and key_inp_shape[-2] == 1:
            permute_order = [
                2,
                3,
                1,
                0,
            ]  # (seq_len, head_dim, 1, 1) -> (1, 1, head_dim, seq_len)
            key_inp = self.mha2sha_optim._op_factory.get_transpose_op(key_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(key_inp)
            head_idx = 3
        elif key_inp_shape[-1] == 1:
            permute_order = [0, 3, 1, 2]  # (B, head_dim, H, 1) -> (B, 1, head_dim, H)
            key_inp = self.mha2sha_optim._op_factory.get_transpose_op(key_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(key_inp)
            head_idx = 3
        elif key_inp_shape[-2] == 1:
            permute_order = [0, 2, 1, 3]  # (B, head_dim, 1, W) -> (B, 1, head_dim, W)
            key_inp = self.mha2sha_optim._op_factory.get_transpose_op(key_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(key_inp)
            head_idx = 2
        else:
        ...
        if key_inp_shape[head_idx] != 1:
            key_inp, key_reshape_init = self.mha2sha_optim._op_factory.get_reshape_op(
                key_inp, [info_dict["batch"], 1, head_dim, -1]
            )  # (B, H, head_dim, W) -> (B, 1, head_dim, H*W)
            self.mha2sha_optim.model.graph.node.append(key_inp)
            self.mha2sha_optim.model.graph.initializer.extend(key_reshape_init)

        _key_inp = key_inp
        _value_inp = value_inp

        if value_inp_shape[0] != info_dict["batch"]:
            batch_idx = value_inp_shape.index(info_dict["batch"])
            head_idx = value_inp_shape.index(1)
            permute_order = [batch_idx, head_idx, 1]
            for idx in range(len(value_inp_shape)):
                if idx not in permute_order:
                    seq_len_idx = idx
            permute_order = [batch_idx, head_idx, seq_len_idx, 1]
            value_inp = self.mha2sha_optim._op_factory.get_transpose_op(value_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(value_inp)
        elif value_inp_shape[-1] == 1 and value_inp_shape[-2] == 1:
            permute_order = [
                2,
                3,
                0,
            ...
            self.mha2sha_optim.model.graph.node.append(value_inp)
            head_idx = 3
        elif value_inp_shape[-1] == 1:
            permute_order = [0, 3, 2, 1]  # (B, head_dim, H, 1) -> (B, 1, H, head_dim)
            value_inp = self.mha2sha_optim._op_factory.get_transpose_op(value_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(value_inp)
            head_idx = 3
        elif value_inp_shape[-2] == 1:
            permute_order = [0, 2, 3, 1]  # (B, head_dim, 1, W) -> (B, 1, W, head_dim)
            value_inp = self.mha2sha_optim._op_factory.get_transpose_op(value_inp, permute_order)
            self.mha2sha_optim.model.graph.node.append(value_inp)
            head_idx = 2
        ...
        if value_inp_shape[head_idx] != 1:
            value_inp, value_reshape_init = self.mha2sha_optim._op_factory.get_reshape_op(
                value_inp, [info_dict["batch"], 1, -1, head_dim]
            )  # (B, head_dim, H, W) -> (B, 1, H*W, head_dim)
            self.mha2sha_optim.model.graph.node.append(value_inp)
            self.mha2sha_optim.model.graph.initializer.extend(value_reshape_init)

        past_value_inp = None
        # Concat past key input and past value output
        if self.mha2sha_optim.handle_past_key_value and self.mha2sha_optim.kv_cache:
            past_key_inp = info_dict["key"]["past_input_name"]
            past_value_inp = info_dict["value"]["past_input_name"]
            # key_transpose: [B, 1, D, L]; value_inp = [B, 1, L, D]
            key_inp, value_inp = self.mha2sha_optim._past_kv_extension.concat_past_key_value_input(
                past_key_inp, past_value_inp, key_inp, value_inp, head_num
            )

        if not self.mha2sha_optim.return_new_key_value_only:
            # Update _key_inp with concat key and value
            _key_inp = key_inp  # [1, 1, emd_dim, past+new seq_len]
            _value_inp = value_inp  # [1, 1, seq_len, past+new emd_dim]

        qk_matmul_node, _ = self.mha2sha_optim._op_factory.get_matmul_op(
            query_inp, key_inp, propose_sha_name + "_qk_MatMul"
        )  # (B, H, W, H*W)
        sha_base_attn_node_list.qk_matmul.append(qk_matmul_node)
        self.mha2sha_optim.model.graph.node.append(qk_matmul_node)
