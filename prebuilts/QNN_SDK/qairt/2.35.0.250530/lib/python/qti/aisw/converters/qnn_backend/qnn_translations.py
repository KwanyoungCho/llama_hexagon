...
class QnnTranslationBase(BackendTranslationBase):
    ...
    def insert_trace_info(self, target_names, target_source_info, graph, target_types=None):
        """
        Function to insert a specific trace info for an op or a tensor from frameworks source info.
        :param target_names: the instance or list of op/tensor names
        :param target_types: the instance or list of target types, it should be same length with target_names,
                             each value should be "op"/"tensor". If not provided or length nod same with target_names,
                             all target will be considered as "tensor" type.
        ...
        :param graph: Instance of op_graph
        """
        if graph.enable_trace:
            if not isinstance(target_names, list):
                target_names = [target_names]
            if target_types is None:
                target_types = ["tensor"] * len(target_names)
            if not isinstance(target_types, list):
                target_types = [target_types]
            if len(target_types) != len(target_names):
                target_types = ["tensor"] * len(target_names)
            if not isinstance(target_source_info, list):
                target_source_info = [target_source_info]
            for ind, tgt_name in enumerate(target_names):
                if target_types[ind] == "op":
                    graph.trace_dict[TraceType.OP][tgt_name] = target_source_info
                else:
                    graph.trace_dict[TraceType.TENSOR][tgt_name] = target_source_info
...
class QnnInputTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        # extract permute order for this tensor
        permute_order_to_src = graph.get_buffer(node.output_names[0]).perm_to_src

        tensor_type = qnn_definitions.QNN_TENSOR_TYPE_APP_WRITE
        custom_quant_params = None
        for entry in graph.user_custom_io:
            if entry['IOName'] == node.output_names[0]:
                if 'QuantParam' in entry:
                    custom_quant_params = entry['QuantParam']
                if 'Optional' in entry and entry['Optional']:
                    tensor_type = qnn_definitions.QNN_TENSOR_TYPE_OPTIONAL_APP_WRITE
        ...
        if graph.user_custom_io and custom_quant_params is not None:
            ...
            quant_params = {
                "definition": qnn_definitions.QNN_DEFINITION_DEFINED,
                "encoding": ir_graph.QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                "scale_offset": {
                    "scale": graph.quantization_params[node.op.name]['output_encodings'][0]['scale'][0],
                    "offset": graph.quantization_params[node.op.name]['output_encodings'][0]['offset'][0]
                    }
            }
            backend.add_custom_input_tensor(
                node.op.name,
                node.output_names[0],
                tensor_type,
                node.op,
                tensor_data_type=numpy_dtype_to_qnn.get(dtype_to_np),
...
class QnnCastTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if output_type == ir_graph.QNN_DATATYPE_FLOAT_32 and backend.float_bitwidth == 16 \
            and not backend.is_online_construction:
            downcast_type = ir_graph.QNN_DATATYPE_FLOAT_16
        elif output_type == ir_graph.QNN_DATATYPE_FLOAT_16 and backend.float_bitwidth == 32:
            downcast_type = ir_graph.QNN_DATATYPE_FLOAT_32
        ...
        if input_type == output_type and graph.has_quantization_param(node.op.name) and 'output_encodings' in graph.get_layer_quantization_param(node.op.name):
            # get quantization encoding of the output tensor of the cast op
            output_encoding = graph.get_layer_quantization_param(node.op.name)['output_encodings'][0]
            output_encoding = output_encoding.copy()
            output_encoding['name'] = node.input_names[0]
            # update the quantization encoding of the input of the cast node
            backend.update_tensor_quant_info(node.input_names[0], output_encoding)

        # Squash Cast op if not output node
        if input_type == output_type and not graph.is_output_node(node):
...
class QnnConstantTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        for consumer in graph.get_buffer(node.output_names[0]).consumers:
            graph.set_trace_info(consumer, graph.get_trace_info([consumer, node, graph.get_buffer(node.output_names[0])]))
        ...
        # Call add_lazy_tensor, if constantOp.constant_tensor has ExternalDataInfo object and not the data loaded
        if (node.op.constant_tensor.is_lazy_data()):
            new_shape =  graph.get_buffer(node.output_names[0]).shape
            backend.add_lazy_tensor(
                node.op.name,
                node.output_names[0],
                qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                new_shape,
                node.op.constant_tensor.get_ext_data_info(),
                check_encodings=check_encodings,
                tensor_data_type=numpy_dtype_to_qnn[node.op.dtype],
                src_axis_format=src_axis_format,
                tensor_axis_format=tensor_axis_format,
                permute_order_to_src=permute_order_to_src,
            ...
            backend.add_tensor(
                ...
                node.op.tensor,
                check_encodings=check_encodings,
                tensor_data_type=numpy_dtype_to_qnn[node.op.tensor.dtype],
                src_axis_format=src_axis_format,
                tensor_axis_format=tensor_axis_format,
                permute_order_to_src=permute_order_to_src,
                orig_tensor_name="tensor",
        ...
        for name in attr.list_names():
            ...
            if attr_type == ir_graph.Qnn_ParamType_t.QNN_PARAMTYPE_SCALAR:
                ...
                if  not backend.serialize_with_suppl_attr and graph.has_buffer(param_tensor_name):
                    ...
                    # extract permute order for this tensor
                    permute_order_to_src = graph.get_buffer(param_tensor_name).perm_to_src
...
class QnnExpandTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        # TODO: consider other way to support expand. change to ElementwiseMul will insert constant tensors
        input_names = node.input_names
        output_names = node.output_names
        ...
        multipler_shape = list(output_shape)
        # Expand op support broadcast based on dimensions are right alignment.
        # Hence it is needed to compare the each dimension's value between output and input from the right to the left.
        for ind in range(-1, -len(input_shape)-1, -1):
            # Only when input and output are equal at the same index, set the "multipler" dimension to 1
            if input_shape[ind] == output_shape[ind]:
                multipler_shape[ind] = 1
        const_op = op_adapter.ConstantOp(const_op_name, np.ones(multipler_shape, qnn_to_numpy_dtype[input_tensor_data_type]))
        ...
        backend.add_tensor(
            ...
            const_op.tensor,
            check_encodings=check_encodings,
            tensor_data_type=input_tensor_data_type,
            src_axis_format=src_axis_format,
            tensor_axis_format=tensor_axis_format,
            permute_order_to_src=permute_order_to_src,
            orig_tensor_name="tensor"
...
@register
class QnnExpandDimsTranslation(QnnTranslationBase):
    TARGET = op_adapter.ExpandDimsOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        axes_tensor_name = backend.create_unique_qnn_tensor_name(node.op.name, qnn_definitions.QNN_OP_EXPAND_DIMS_PARAM_AXES)
        axes = node.op.axes
        ...
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_EXPAND_DIMS,
                         node.input_names,
                         backend.get_outputs_info(node, graph),
                         tensor_params={qnn_definitions.QNN_OP_EXPAND_DIMS_PARAM_AXES: axes_tensor_info},
                         macs=node.op.macs)
...
@register
class QnnIsInfTranslation(QnnTranslationBase):
    TARGET = op_adapter.IsInfOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        detect_positive = node.op.detect_positive
        detect_negative = node.op.detect_negative
        tensor_params = {}
        scalar_params = {ir_graph.QNN_OP_IS_INF_PARAM_DETECT_NEGATIVE: (numpy_dtype_to_qnn[np.dtype('bool')], np.bool_(detect_negative)),
                         ir_graph.QNN_OP_IS_INF_PARAM_DETECT_POSITIVE: (numpy_dtype_to_qnn[np.dtype('bool')], np.bool_(detect_positive)),}

        output_tensor_dtype = ir_graph.QNN_DATATYPE_BOOL_8
        check_encodings = False
        backend.add_node(node.op.name, ir_graph.QNN_OP_IS_INF,
                         node.input_names,
                         backend.get_outputs_info(node, graph, tensor_data_type=output_tensor_dtype,
                                                  check_encodings=check_encodings),
                                                  scalar_params=scalar_params,
                                                  tensor_params=tensor_params)
...
@register
class QnnCombinedNmsTranslation(QnnTranslationBase):
    TARGET = op_adapter.CombinedNmsOp.TRANSLATION_KEY
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        for i, output_name in enumerate(output_names):
            ...
            if i == 2:
                ...
                # boxes and scores
                tensor_data_type = backend.retrieve_tensor_info(node.input_names[i])["data_type"]
                if i != 0:
                    # scores
                    check_encodings = True
            output_tensor_info = backend.get_output_info(graph,
        ...
        scalar_params = {
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_MAX_BOXES_PER_CLASS:
                (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(node.op.max_boxes_per_class)),
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_MAX_TOTAL_BOXES:
                (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(node.op.max_total_boxes)),
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_IOU_THRESHOLD:
                (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(node.op.iou_threshold)),
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_SCORE_THRESHOLD:
                (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(node.op.score_threshold)),
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_PAD_PER_CLASS:
                (numpy_dtype_to_qnn[np.dtype('bool')], np.float32(node.op.pad_per_class)),
            ir_graph.QNN_OP_COMBINED_NMS_PARAM_CLIP_BOXES:
                (numpy_dtype_to_qnn[np.dtype('bool')], np.float32(node.op.clip_boxes))
        }

        backend.add_node(node.op.name, ir_graph.QNN_OP_COMBINED_NMS,
                         node.input_names,
                         outputs_info,
                         scalar_params=scalar_params)
...
class QnnScatterElementsTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        supported_reductions = {ir_graph.QNN_OP_SCATTER_ELEMENTS_REDUCTION_NONE: qnn_definitions.QNN_OP_SCATTER_ELEMENTS_REDUCTION_NONE,
                                ...
                                ir_graph.QNN_OP_SCATTER_ELEMENTS_REDUCTION_MAX: qnn_definitions.QNN_OP_SCATTER_ELEMENTS_REDUCTION_MAX}
...
@register
class QnnSqueezeTranslation(QnnTranslationBase):
    TARGET = op_adapter.SqueezeOp.TRANSLATION_KEY

    def add_op_to_backend(self, node, graph, backend, **kwargs):
        axes_tensor_name = backend.create_unique_qnn_tensor_name(node.op.name, qnn_definitions.QNN_OP_SQUEEZE_PARAM_AXES)
        axes = node.op.axes
        axes_tensor_info = backend.create_tensor_info(axes_tensor_name, qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                                                      [len(axes)], ir_graph.QNN_DATATYPE_UINT_32,
                                                      data=np.asarray(axes, dtype=np.uint32))

        backend.add_node(node.op.name, qnn_definitions.QNN_OP_SQUEEZE,
                         node.input_names,
                         backend.get_outputs_info(node, graph),
                         tensor_params={qnn_definitions.QNN_OP_SQUEEZE_PARAM_AXES: axes_tensor_info},
                         macs=node.op.macs)
