...
class DLCQuantizer(object):
    ...
    def __init__(self,
        ...
        if backend_info_obj:
            if backend_info_obj.backend_name().lower() != "htp":
    ...
    def make_tensors_updateable(self, tensor_names):
        tensor_map = self.ir_graph.get_tensor_map()
        for tensor_name in tensor_names:
            if not self.ir_graph.has_tensor(tensor_name):
                raise RuntimeError("Attempting to make tensor, {}, updateable but is not in the graph".format(tensor_name))

            tensor = tensor_map[tensor_name]
            # reset the original updatable tensors to updatable
            log_debug1("Resetting the tensor {} to updatable".format(tensor_name))
            tensor.set_updateable(True)

    def make_activation_and_bias_tensors_updateable(self):
    ...
    def quantize(self):
        ...
        # IrQuantizer -> Calibration + Apply Encodings
        # IrQuantizerV2 -> Apply Encodings (uses externally calibrated encodings)
        # use_quantize_v2 is currently only enabled for QNN_INTERNAL_DEV build
        if hasattr(self, 'use_quantize_v2') and self.use_quantize_v2:
            log_error("Apply Encodings cannot be invoked from qairt-quantizer. Use qairt-converter with "
                      "v2.0.0.beta encodings instead.")
            quantizer = ir_quantizer.IrQuantizerV2(self.opts_v2, self.ir_graph)
        ...
        if self.lora_enabled:
            self.make_tensors_updateable(saved_updatable_tensor_names)

            if not self.ir_graph.is_partial_quant_updateable():
                self.make_activation_and_bias_tensors_updateable()
