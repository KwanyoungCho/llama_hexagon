...
from mha2sha.utils.base_arch_configs import base_archs_to_flags

from mha2sha.utils.logger import log_debug, log_error, log_warning
...
class ExplicitlySetArgumentParser(argparse.ArgumentParser):
    ...
    def parse_args(self, *args, **kwargs):
        ...
        if args:
            ...
            self.explicitly_set_args.update({unflagify(arg) for arg in args})

        return return_args

    def parse_known_args(self, *args, **kwargs):
        return_args, unknown = super().parse_known_args(*args, **kwargs)
...
def converter_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mha2sha",
        action="store_true",
        help="Enable MHA2SHA optimization pass",
    )
    parser.add_argument(
        "--build-ar",
        type=check_ar_value,
...
def pretty_print_args(args_: dict, model_byte_size: str) -> None:
    ...
    byte_size_caption = bold_text(" Model byte size: ") + bold_text(
        f"{model_byte_size:,}", color=PrettyPrintConstants.Q_BLUE
    )
    version_caption = bold_text(" Version: ") + bold_text(
        __version__, color=PrettyPrintConstants.Q_BLUE
    )
    ...
    for arg, val in args_.items():
        ...
        # If a user explicitly set a flag and 'base_arch' was set, then
        # we want to warn the user in the table.
        if args_["base_arch"] and arg in (
...
def pretty_print_warning_table(
    console, base_arch: str, overwritten_args: Dict[str, Tuple[str]]
) -> None:
    """Pretty prints a warning table of user overwritten flags.

    If a user passes in the 'base_arch' and overwrites a flag,
    ...
    """
    warning_explanation = """
    The [italic]--base-arch[/] flag is used to turn on necessary flags for specific model
    architectures. For example, [italic]--base-arch llama2[/] would implictly turn on flags
    ...
    try:
        table = create_rich_table(
            ...
            title=f":warning:  --base-arch '{base_arch}' Mapped Args Overwritten :warning:",
            headers=["Flag", f"'{base_arch}' Default", "User Provided"],
    ...
    console.print(rich.panel.Panel(warning_explanation, width=table.width))
...
AvailableConverterArgs = Enum(
    ...
    {arg_name.upper(): arg_name for arg_name in _args_to_default_value.keys()},
)

AvailableConverterArgs.default_value = lambda self: _args_to_default_value[
    self.value.lower()
]


def filter_args(explicitly_set_args, **passed_in_args) -> Dict:
    """Filters passed in args and returns the args updated

    If a user passes in '--base-arch' we will turn on the corresponding models
    flags. However, if a user explicitly passes in a value that conflicts
    against the base-arch, we will NOT update that flag.
    ...
    """
    args_to_iter = list(AvailableConverterArgs)

    args_to_return = {}

    if not explicitly_set_args:
        explicitly_set_args.update({arg_name for arg_name in passed_in_args.keys()})

    def override_arg(x):
        return x not in explicitly_set_args

    base_arch = passed_in_args.pop("base_arch", None)
    args_to_return["base_arch"] = base_arch
    ...
    for available_arg in args_to_iter:
        ...
        # Second, if we have a "base-arch" flag passed in then we can
        # override an arg if that arg is NOT in the explicitly set args.
        # Otherwise, we set what the user passed in.
        if base_arch:
            base_arch_val = base_archs_to_flags[base_arch].get(arg_name, None)
            if base_arch_val is not None:
                if override_arg(arg_name):
                    log_debug(f"{flagify(arg_name)} overridden to '{base_arch_val}'")
                    args_to_return[arg_name] = base_arch_val
                else:
                    log_warning(
                        f"Flag {flagify(arg_name)} is set to '{base_arch_val}' "
                        f"for '{base_arch}' but got '{passed_in_arg_val}' "
