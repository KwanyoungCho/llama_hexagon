...
def clean_model(model: ModelProto) -> None:
    """
    ...
    unused_initializers = [init for init in graph.initializer if init.name not in node_by_input_name]
    ...
    unused_value_infos = [
        val_info for val_info in graph.value_info if val_info.name not in node_by_input_name
    ]
    ...
    remove_unused_inputs_outputs(model)


def get_graph_by_initializer(model: ModelProto, init: TensorProto) -> Union[None, GraphProto]:
    """
...
def remove_unused_inputs_outputs(model: ModelProto) -> None:
    """
    ...
    remove_graph_outputs = [g_op for g_op in main_graph.output if is_dangling_graph_output(g_op.name)]
    remove_graph_inputs = [g_op for g_op in main_graph.input if is_dangling_graph_input(g_op.name)]
...
def topological_sort(model: ModelProto) -> None:
    """
