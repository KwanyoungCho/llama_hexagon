...
from .axis_format import AxisFormat
...
def determine_layout(tensor_shape: Union[Tuple[int], List[int]]) -> Text:
    ...
    if not isinstance(tensor_shape, Tuple) and not isinstance(tensor_shape, List):
        log_debug(
            "Tensor Layout cant be obtained from " f"tensor shape: {tensor_shape}"
        )
    ...
    if len(tensor_shape) == 4:
        ...
        log_debug("Cannot determine layout for tensor " f"with shape: {tensor_shape}.")
        return layout.NONTRIVIAL
