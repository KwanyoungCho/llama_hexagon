...
from typing import IO, Callable, Dict, Iterable, List, Optional, Set, Text, Tuple, Union
...
onnx_log_area = LogAreas.register_log_area("Onnx")
logger = QAIRTLogger.register_area_logger(
    onnx_log_area, level="INFO", formatter_val="extended", handler_list=["dev_console"]
)
...
def check_duplicates_in_attribute(graph: GraphProto, attribute_name: str) -> bool:
    ...
    for property in list_of_attributes:
        if property.name != "" and property.name not in property_name_set:
            ...
            if property.name == "":
                logger.debug("Graph checker: No {} name found for.".format(attribute_name))
            else:
                logger.debug(f"Graph checker: {attribute_name} '{property.name}' is duplicate.")
...
def get_parent_at_any_level(
    output_name: str,
    get_node_by_output_name: Dict[str, NodeProto],
    level: int = 1,
) -> List[NodeProto]:
    """
    ...
    if level == 1:
        ...
        logger.error(f"Can't find the parent at given level: {level}")
        return None
...
def get_shape_from_value_info_proto(
    ...
    if not tensor_type.HasField("shape"):
        logger.warning(f"Shape not found for tensor: {val_info.name}")
        return tensor_shape
...
def graph_checker(model: ModelProto) -> bool:
    ...
    node_check_status = True
    for graph in get_graphs(model):
        node_check_status = node_check_status and check_duplicates_in_attribute(graph, "node")
        node_check_status = node_check_status and check_duplicates_in_attribute(graph, "input")
        node_check_status = node_check_status and check_duplicates_in_attribute(graph, "output")
    return node_check_status
...
class ONNXPatternMatcher:
    ...
    def __parse_model(self):
        ...
        4. Input: GraphProto
        """
        self.graph_output_node_names = [n.name for n in self.model.graph.output]
        cnt_n = [n for n in get_nodes(self.model) if n.name == ""]
        if len(cnt_n) > 1:
            logger.warning(
                "More than one nodes with same name : '' found. Pattern "
                "Matcher requires unique node names for all node. "
                "Assigning new names to such nodes."
            )
            ...
            if n.op_type == pattern[0]:
                ...
                while len(stack) != 0:
                    ...
                    if _node.op_type == pattern[_node_level]:
                        ...
                        stack.extend([[op, _node_level + 1] for op in all_node_op_nodes])
...
def get_pattern_start_end_nodes(model: ModelProto, attention_patterns: Dict) -> Tuple[List, List, List]:
    """
...
def get_children(node, get_node_by_input_name):
    ...
    for out in node.output:
        ...
        return im_children_nodes


def get_least_commom_ancestor_with_verified_pathway(node_1, node_2, optimizer, pathway_nodes_verifier=None):
    """
    ...
    node_1_input = node_1.input
    node_2_input = node_2.input
    lca = {}
    ...
    while not lca:
        ...
        node_1_ancestors_name.update(node_1_parents_name)
        ...
        node_2_ancestors_name.update(node_2_parents_name)
...
def _get_next_node_base_on_cond_helper(
    ...
    while stack:
        ...
        if node_end_search_cond and node_end_search_cond(node):
            logger.debug(f"Node: '{node.name}' met end search criteria.")
            continue

        next_nodes = list(filter(None, get_next_nodes(node, get_node_by_input_or_output_name)))

        if not next_nodes:
            logger.debug(f"No {'children' if search_down else 'parent'} nodes found for node: '{node.name}'")
            continue
...
def get_next_node_down_based_on_cond(
    start_node, get_node_by_input_name, *, node_found_cond, node_end_search_cond=None
):
    """
...
def get_next_node_up_based_on_cond(
    start_node, get_node_by_output_name, *, node_found_cond, node_end_search_cond=None
):
    """
...
def get_initializer_value(init_node):
    const_tensor_value = None
    if raw_data := getattr(init_node, "raw_data", None):
        np_dtype = helper.tensor_dtype_to_np_dtype(init_node.data_type)
        const_tensor_value = np.frombuffer(raw_data, dtype=np_dtype).reshape(init_node.dims)
    if const_tensor_value is None:
        field = helper.tensor_dtype_to_field(init_node.data_type)
        const_tensor_value = getattr(init_node, field)
    return const_tensor_value


def get_add_value(add_node, get_initializer_by_name, get_node_by_output_name):
    add_value = None
    for add_input in add_node.input:
        if add_input in get_initializer_by_name:
            add_init = get_initializer_by_name[add_input]
            add_value = get_initializer_value(add_init)

        elif add_input in get_node_by_output_name:
            const_node = get_node_by_output_name[add_input]
            if const_node.op_type == "Identity":
                add_init = get_initializer_by_name[const_node.input[0]]
                add_value = get_initializer_value(add_init)

            elif const_node.op_type == "Constant":
                add_value = get_constant_node_value(const_node)
...
def get_pad_info(pad_node, get_node_by_output_name, get_initializer_by_name):
    """Get pads and constant value if available from pad node."""
    if pad_node.input[1] in get_initializer_by_name:
        pads = get_initializer_value(get_initializer_by_name[pad_node.input[1]])
    else:
        pads = get_constant_node_value(get_node_by_output_name[pad_node.input[1]])

    constant_val = None
    if onnx.helper.get_node_attr_value(pad_node, "mode").decode("utf-8") == "constant":
        if pad_node.input[2] in get_initializer_by_name:
            constant_val = get_initializer_value(get_initializer_by_name[pad_node.input[2]])
        else:
            constant_val = get_constant_node_value(get_node_by_output_name[pad_node.input[2]])
    return pads, constant_val.item()


def get_slice_info(slice_node, get_node_by_output_name, get_initializer_by_name):
    """Get start, end, axis, step from slice node."""
    if slice_node.input[1] in get_initializer_by_name:
        start = get_initializer_value(get_initializer_by_name[slice_node.input[1]])[0]
    else:
        start = get_constant_node_value(get_node_by_output_name[slice_node.input[1]])[0]
    if slice_node.input[2] in get_initializer_by_name:
        end = get_initializer_value(get_initializer_by_name[slice_node.input[2]])[0]
    else:
        end = get_constant_node_value(get_node_by_output_name[slice_node.input[2]])[0]
    if slice_node.input[3] in get_initializer_by_name:
        axes = get_initializer_value(get_initializer_by_name[slice_node.input[3]])[0]
    else:
        axes = get_constant_node_value(get_node_by_output_name[slice_node.input[3]])[0]
    step = 1
    if len(slice_node.input) == 5:
        if slice_node.input[4] in get_initializer_by_name:
            step = get_initializer_value(get_initializer_by_name[slice_node.input[4]])[0]
        else:
            step = get_constant_node_value(get_node_by_output_name[slice_node.input[4]])[0]
    return start, end, axes, step
...
def get_node_input_constant_op_value(node, get_node_by_output_name, get_initializer_by_name):
    ...
    if (
        ...
        const_tensor_value = get_constant_node_value(get_node_by_output_name[node.input[0]])
        # shape_const_node = get_node_by_output_name[node.input[0]]
    elif node.input[0] in get_initializer_by_name:
        const_tensor_value = get_initializer_value(get_initializer_by_name[node.input[0]])
    elif (
        node.input[1] in get_node_by_output_name.keys()
        and get_node_by_output_name[node.input[1]].op_type == "Constant"
    ):
        const_tensor_value = get_constant_node_value(get_node_by_output_name[node.input[1]])
        # shape_const_node = get_node_by_output_name[node.input[1]]
    elif node.input[1] in get_initializer_by_name:
        const_tensor_value = get_initializer_value(get_initializer_by_name[node.input[1]])
    else:
        raise ValueError(f"Expecting one of the reshape's input type to be Constant or an Intializer, \
...
def load_model(model_or_path: str) -> Tuple[ModelProto, str]:
    logger.info(f"Loading the onnx model from: {model_or_path}")
...
def save_model(model: ModelProto, f: IO[bytes] | str | os.PathLike | Path) -> None:
    """Saves an ONNX model.
    ...
    if get_model_size(model) >= 2:
        logger.info("Model is over 2GB. Saving model with `save_as_external_data`")
        dirname = f.parent
...
def native_checker(model: ModelProto) -> bool:
    ...
    if not graph_check_status:
        logger.warning("Duplicate naming found in the graph.")
    ...
    try:
        ...
        logger.warning("The model is invalid: %s" % e)
        return False
