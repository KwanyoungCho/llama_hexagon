...
class QnnOpPackageGenerator:
    ...
    def parse_config_to_op_def_collection(self, config_path: str):
        """
        Parses a user provided json config into an OpDefCollection object. The config is expected to contain information
        about a user's operation, as well as additional fields about the package.
        :param config_path: The file path to the user's json config file, which must be a valid XML file.
        """
        try:
        ...
        return config_path, op_collection
    ...
    def setup_backend_specific_file_paths(self, package_info, package_handler, gen_cmakelists=False):
        ...
        if package_info.backend == "CPU" or package_info.backend == "GPU":
            ...
            if package_info.backend == "CPU":
                ...
                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), 'Makefile.qhs225-aarch64')):
                    makefile_handler = FileHandler('Makefile.qhs225-aarch64',
                                                   make_dir_handler.dir_abs_path,
                                                   copy_source_location=os.path.join(MAKEFILE_PATH,
                                                                                     package_info.backend.upper(),
                                                                                     'Makefile.qhs225-aarch64'))
                    make_dir_handler.file_handlers.extend([makefile_handler])

                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), 'Makefile.qos226-aarch64')):
                    makefile_handler = FileHandler('Makefile.qos226-aarch64',
                                                   make_dir_handler.dir_abs_path,
                                                   copy_source_location=os.path.join(MAKEFILE_PATH,
                                                                                     package_info.backend.upper(),
                                                                                     'Makefile.qos226-aarch64'))
                    make_dir_handler.file_handlers.extend([makefile_handler])

                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), 'Makefile.qos227-aarch64')):
                    makefile_handler = FileHandler('Makefile.qos227-aarch64',
                                                   make_dir_handler.dir_abs_path,
                                                   copy_source_location=os.path.join(MAKEFILE_PATH,
                                                                                     package_info.backend.upper(),
                                                                                     'Makefile.qos227-aarch64'))
                    make_dir_handler.file_handlers.extend([makefile_handler])

                if os.path.isfile(os.path.join(MAKEFILE_PATH, package_info.backend.upper(), 'Makefile.qhs227-aarch64')):
                    makefile_handler = FileHandler('Makefile.qhs227-aarch64',
                                                   make_dir_handler.dir_abs_path,
                                                   copy_source_location=os.path.join(MAKEFILE_PATH,
                                                                                     package_info.backend.upper(),
                                                                                     'Makefile.qhs227-aarch64'))
                    make_dir_handler.file_handlers.extend([makefile_handler])
