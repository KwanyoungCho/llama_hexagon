...
class QnnConverterBackendBase(ConverterBackend):
    ...
    def process_float_datatype(self, is_bias=False, is_qairt=False):
        # Force tensor datatype as Float32 for QAIRT if encodings are present
        # This datatype behavior will be handled later during Float Fallback call
        if (self.is_online_construction or self.float_bitwidth == 32 or
           (self.float_bias_bitwidth == 32 and is_bias) or
           (self.encodings_present and is_qairt)):
    ...
    def create_tensor_info(
        ...
        if quantizable and self.is_float(tensor_data_type):
            tensor_data_type = self.process_float_datatype(is_bias, self.qairt_converter)
        ...
        if permute_order_to_src is not None:
            tensor_info["permute_order_to_src"] = permute_order_to_src
        else:
            tensor_info["permute_order_to_src"] = []
    ...
    def get_output_info(self, graph, tensor_name, tensor_type=qnn_definitions.QNN_TENSOR_TYPE_NATIVE, tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32,
        ...
        output_tensor_info = self.create_tensor_info(
            tensor_name,
            tensor_type,
            tensor_shape,
            tensor_data_type,
            src_axis_format,
            tensor_axis_format,
            permute_order_to_src=output_buf.perm_to_src,
            encoding=encoding,
    ...
    def get_ir_graph(self, graph):
        ...
        try:
            ...
            self.encodings_present = False
            if graph.quantization_params:
                self.model.set_tensor_overrides(graph.quantization_params)
                self.encodings_present = any(q['output_encodings'] or q['param_encodings'] for q in graph.quantization_params.values())
            ...
            if self.qairt_converter and self.input_network.split(".")[-1]=='onnx':
                self.model.set_graph_output_tensors_order(graph.output_names)
