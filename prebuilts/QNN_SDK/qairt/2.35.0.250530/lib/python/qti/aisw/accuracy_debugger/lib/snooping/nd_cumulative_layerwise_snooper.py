...
from qti.aisw.accuracy_debugger.lib.utils.snooper_utils import show_progress, LayerStatus,\
    files_to_compare, append_to_intermediate_report, SnooperUtils as su
...
from qti.aisw.accuracy_debugger.lib.utils.nd_verifier_utility import permute_tensor_data_axis_order, get_irgraph_tensors_info
...
class CumulativeLayerwiseSnooping(Snooper):
    ...
    def __init__(self, args, logger, verbose="info"):
        super(CumulativeLayerwiseSnooping, self).__init__(snooping_type="cumulative_layerwise",
                                                          args=args, logger=logger, verbose=verbose)
    ...
    def run(self, model=None, profile_info=None):
        ...
        layerwise_intermediate_report_path = self.output_dir + '/intermediate_cumulative_layerwise.csv'
        layerwise_report_path = self.output_dir + '/cumulative_layerwise.csv'

        #Change relative paths in input list to absolute paths
        self.input_list_file = self._set_input_list(self.input_list_file)
        ...
        quantization_overrides_file_path = self.extract_and_filter_encodings(self.output_dir, model)
        self.quantization_overrides = quantization_overrides_file_path
        ...
        while (True):
            ...
            if not skip_compare:
                ...
                qnn_model_json_path = os.path.join(self.output_dir, 'inference_engine',
                                                   s_cur_layer_out_name, 'model_net.json')

                # Permute target tensor to align with golden tensor
                tensors_info = get_irgraph_tensors_info(qnn_model_json_path=qnn_model_json_path)
                if tensors_info is not None and s_cur_layer_out_name in tensors_info:
                    inf_raw, is_permuted = permute_tensor_data_axis_order(
                        inf_raw, tensors_info[s_cur_layer_out_name])
                ...
                for elem in original_outputs:
                    ...
                    if original_output_name not in self.is_transpose_needed_dict:
                        # Permute target tensor to align with golden tensor
                        tensors_info = get_irgraph_tensors_info(
                            qnn_model_json_path=qnn_model_json_path)
                        if tensors_info is not None and s_original_output_name in tensors_info:
                            org_inf_raw, is_permuted = permute_tensor_data_axis_order(
                                org_inf_raw, tensors_info[s_original_output_name])
                ...
                self.logger.info(
                    f"Intermediate results are stored in {layerwise_intermediate_report_path}")
                append_to_intermediate_report(layerwise_intermediate_report_path, layer_name,
                                              cur_layer_out_name, percent_match)
        ...
        print('Summary report saved at {}'.format(layerwise_report_path))

        # Plot verifier scores
        plots_data = {}
        # Gather current layer outputs scores
        for verifier_name in overall_comp_list:
            plots_data[f'{verifier_name}(current_layer)'] = df[verifier_name].values

        # Gather original outputs scores
        for data in df['Orig Outputs'].values:
            for output_name in data.keys():
                for comparator, score in data[output_name].items():
                    plot_name = f'{comparator}({output_name})'
                    if plot_name not in plots_data.keys():
                        plots_data[plot_name] = [score]
                    else:
                        plots_data[plot_name] += [score]

        plots_save_dir = os.path.join(self.output_dir, 'plots')
        os.makedirs(plots_save_dir, exist_ok=True)
        for verifier_name, scores in plots_data.items():
            try:
                self.logger.debug(f'Plotting graph for {verifier_name} scores...')
                Visualizers.line_plot(x=df.index.values, y=scores, plot_name=verifier_name,
                                      save_dir=plots_save_dir)
            except Exception as e:
                self.logger.warning(f'Plotting graph failed with error: {e}')
        self.logger.info(f'Summary plots saved at {plots_save_dir}')
