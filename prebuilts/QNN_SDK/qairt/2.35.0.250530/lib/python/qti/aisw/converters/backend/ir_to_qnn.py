...
class QnnConverterBackend(QnnConverterBackendBase):
    ...
    def add_custom_input_tensor(
        ...
        if quant_params:
            ...
            elif tensor_data_type == ir_graph.QNN_DATATYPE_UINT_16:
                tensor_data_type = ir_graph.QNN_DATATYPE_UFIXED_POINT_16
            elif tensor_data_type == ir_graph.QNN_DATATYPE_INT_16:
                tensor_data_type = ir_graph.QNN_DATATYPE_SFIXED_POINT_16
    ...
    def save(self, graph):
        ...
        graph_input_names = [node.output_names[0] for node in graph.get_input_nodes_to_graph()]
        if graph.preserve_io_datatype_passed == 2:
            self.quantizer.opts.use_native_input_dtype = True
        elif graph.preserve_io_datatype_passed == 1:
            native_input_tensor_names = [name for name in list(graph.preserve_datatype_tensors.keys()) if name in graph_input_names]
            if len(native_input_tensor_names) > 0:
                graph_name = os.path.basename(self.output_path).split('.')[0]
                self.quantizer.opts.native_input_tensor_names = graph_name + ":" + ",".join(native_input_tensor_names)
        ...
        if graph.enable_trace and self.quantizer.opts.input_list:
            ir_graph.validation_framework_tracing_completeness("IrQuantizer")
