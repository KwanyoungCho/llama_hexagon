...
PARAM_ENCODING_KEYS = ["q_proj_param", "k_proj_param", "v_proj_param"]
...
class BaseAttnNode:
    ...
    lora_q: List[LoraNode] | None
    lora_k: List[LoraNode] | None
    lora_v: List[LoraNode] | None
...
def create_base_attn_mapping_dict(mha_base_attn_node) -> Dict[str, NodeMappingDict]:
    ...
    return {
        ...
        "qk_scale": create_activation_node_mapping_dict(
            input_1_name=None,
            input_2_name=mha_base_attn_node.qk_scale.input[1]
            if mha_base_attn_node.qk_scale is not None
            else None,
            output_name=mha_base_attn_node.qk_scale.output[0]
            if mha_base_attn_node.qk_scale is not None
            else None,
        ),
        "qk_mask_add": create_activation_node_mapping_dict(
            input_1_name=None,
            input_2_name=None,
            output_name=mha_base_attn_node.qk_mask_add.output[0]
            if mha_base_attn_node.qk_mask_add is not None
            else None,
        ),
        "qk_alibi_add": create_activation_node_mapping_dict(
            input_1_name=None,
            input_2_name=mha_base_attn_node.qk_alibi_add.input[1]
            if mha_base_attn_node.qk_alibi_add is not None
            else None,
            output_name=mha_base_attn_node.qk_alibi_add.output[0]
            if mha_base_attn_node.qk_alibi_add is not None
            else None,
        ),
        "qk_softmax": create_activation_node_mapping_dict(
            ...
            if mha_base_attn_node.qk_softmax is not None
            else None,
        ),
        ...
        "q_proj_activation": create_activation_node_mapping_dict(
            input_1_name=None, input_2_name=None, output_name=mha_base_attn_node.q_matmul.output[0]
        ),
        "k_proj_activation": create_activation_node_mapping_dict(
            input_1_name=None, input_2_name=None, output_name=mha_base_attn_node.k_matmul.output[0]
        ),
        "v_proj_activation": create_activation_node_mapping_dict(
            input_1_name=None, input_2_name=None, output_name=mha_base_attn_node.v_matmul.output[0]
        ),
...
def update_base_attn_sha_encoding_name_to_base_attn_encoding_mapping_dict(
    base_attn_mapping_dict, sha_base_attn_node_list
):
    """
    ...
    for _key, _attr in base_mapping_dict_keys_node_attr_pair:
        if _key == "qkv_head_concat":
            update_multi_input_concat_sha_tensor_to_node_mapping_dict(
                base_attn_mapping_dict[_key], getattr(sha_base_attn_node_list, _attr)[0]
            )
        else:
            update_sha_tensor_to_node_mapping_dict(
                base_attn_mapping_dict[_key], getattr(sha_base_attn_node_list, _attr)
            )
