...
def create_branch_rope_encoding_mapping_dict(
    mha_rope_dict: RopeNode, prefix=None
...
def update_rope_sha_encoding_name_to_rope_encoding_mapping_dict(
    rope_encoding_mapping_dict, q_sha_rope_node, k_sha_rope_node
):
    """
    ...
    """
    for rope_branch_prefix, sha_rope_node in zip(
        ROPE_BRANCH_PREFIX, [q_sha_rope_node, k_sha_rope_node]
    ):
        for rope_node_name in ROPE_ACTIVATION_ENCODING_KEYS:
            if (ROPE_BRANCH_PREFIX[1] == rope_branch_prefix) and (
                f"{rope_branch_prefix}_{rope_node_name}"
                not in rope_encoding_mapping_dict
            ):
                log_debug(
                    f"Missing {rope_branch_prefix}_{rope_node_name} in RopeNode. Expecting BERT model"
                )
                continue
...
class RopeExtension:
    ...
    def create_llama_rope_node(
        ...
        if self.mha2sha_optim.nchw_aligned:
            # inp shape: [1, head_dim, 1, seq_len], slice half on dim = 1: [1, head_dim//2, 1, seq_len] -> transpose to [1, 1, seq_len, head_dim//2]
            _1st_half_slice, _1st_slice_init_list = (
                self.mha2sha_optim._op_factory.get_slice_op(
                    inp, start=0, end=head_dim // 2, axis=1
                )
            )
            _2nd_half_slice, _2nd_slice_init_list = (
                self.mha2sha_optim._op_factory.get_slice_op(
                    inp, start=head_dim // 2, end=head_dim, axis=1
                )
            )
            self.mha2sha_optim.model.graph.initializer.extend(
                _1st_slice_init_list + _2nd_slice_init_list
            )
            self.mha2sha_optim.model.graph.node.extend(
                [_1st_half_slice, _2nd_half_slice]
            )

            _1st_half_transpose = self.mha2sha_optim._op_factory.get_transpose_op(
                _1st_half_slice, [0, 2, 3, 1]
            )
            _2nd_half_transpose = self.mha2sha_optim._op_factory.get_transpose_op(
                _2nd_half_slice, [0, 2, 3, 1]
            )
            self.mha2sha_optim.model.graph.node.extend(
                [_1st_half_transpose, _2nd_half_transpose]
            )
            ...
            _1st_half_slice, _1st_slice_init_list = (
                self.mha2sha_optim._op_factory.get_slice_op(
                    inp, start=0, end=head_dim // 2, axis=3
                )
            )
            _2nd_half_slice, _2nd_slice_init_list = (
                self.mha2sha_optim._op_factory.get_slice_op(
                    inp, start=head_dim // 2, end=head_dim, axis=3
                )
            )
            self.mha2sha_optim.model.graph.initializer.extend(
                _1st_slice_init_list + _2nd_slice_init_list
        ...
        _1st_cos_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            _1st_half_slice, rope_cos
        )
        _2nd_sin_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            _2nd_half_slice, rope_sin
        )
        embed_1st_sub = self.mha2sha_optim._op_factory.get_sub_op(
            _1st_cos_mul, _2nd_sin_mul
        )

        # Embedded 2nd half: 1st_half*sin + 2nd_half*cos
        _1st_sin_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            _1st_half_slice, rope_sin
        )
        _2nd_cos_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            _2nd_half_slice, rope_cos
        )
        embed_2nd_add = self.mha2sha_optim._op_factory.get_add_op(
            _1st_sin_mul, _2nd_cos_mul
        )

        inp_concat_node = self.mha2sha_optim._op_factory.get_concat_op(
            [embed_1st_sub, embed_2nd_add], 3
        )
    ...
    def get_llama_rope_node_without_positional_embedding(
        self,
        query_inp: NodeProto,
        key_inp: NodeProto,
        rope_cos: NodeProto,
        ...
        if self.mha2sha_optim.use_position_embedding:
            input_cos_tensor = self.mha2sha_optim.model.graph.input[
                self.mha2sha_optim.mha_model_input_names_index_dict[rope_cos]
            ]
            if (
                input_cos_tensor.type.tensor_type.shape.dim[-1].dim_value
                == head_dim // 2
            ):
                cos_concat_node = self.mha2sha_optim._op_factory.get_concat_op(
                    [rope_cos, rope_cos], -1
                )
            else:
                raise ValueError(
                    f"Expecting prerared llama cos have dim: {head_dim//2} on it's last dim but got {[input_cos_tensor.type.tensor_type.shape.dim[-1].dim_value]}"
                )

            input_sin_tensor = self.mha2sha_optim.model.graph.input[
                self.mha2sha_optim.mha_model_input_names_index_dict[rope_cos]
            ]
            if (
                input_sin_tensor.type.tensor_type.shape.dim[-1].dim_value
                == head_dim // 2
            ):
                sin_concat_node = self.mha2sha_optim._op_factory.get_concat_op(
                    [rope_sin, rope_sin], -1
                )
            else:
                raise ValueError(
                    f"Expecting prerared llama sin have dim: {head_dim//2} on it's last dim but got {[input_sin_tensor.type.tensor_type.shape.dim[-1].dim_value]}"
                )

            self.mha2sha_optim.model.graph.node.extend(
                [cos_concat_node, sin_concat_node]
            )
            rope_cos = cos_concat_node
            rope_sin = sin_concat_node

        # Rope on query
        q_1st_half, q_1st_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(
            query_inp, start=0, end=head_dim // 2, axis=3
        )
        q_2nd_half, q_2nd_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(
            query_inp, start=head_dim // 2, end=head_dim, axis=3
        )
        q_2nd_half_neg = self.mha2sha_optim._op_factory.get_neg_op(q_2nd_half)
        q_concat_node = self.mha2sha_optim._op_factory.get_concat_op(
            [q_2nd_half_neg, q_1st_half], -1
        )
        q_cos_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            query_inp, rope_cos
        )
        q_sin_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            q_concat_node, rope_sin
        )
        rope_embed_query_inp = self.mha2sha_optim._op_factory.get_add_op(
            q_cos_mul, q_sin_mul
        )
        ...
        # Rope on keys
        k_1st_half, k_1st_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(
            key_inp, start=0, end=head_dim // 2, axis=3
        )
        k_2nd_half, k_2nd_slice_init_list = self.mha2sha_optim._op_factory.get_slice_op(
            key_inp, start=head_dim // 2, end=head_dim, axis=3
        )
        k_2nd_half_neg = self.mha2sha_optim._op_factory.get_neg_op(k_2nd_half)
        k_concat_node = self.mha2sha_optim._op_factory.get_concat_op(
            [k_2nd_half_neg, k_1st_half], -1
        )
        k_cos_mul = self.mha2sha_optim._op_factory.get_element_mul_op(key_inp, rope_cos)
        k_sin_mul = self.mha2sha_optim._op_factory.get_element_mul_op(
            k_concat_node, rope_sin
        )
        rope_embed_key_inp = self.mha2sha_optim._op_factory.get_add_op(
            k_cos_mul, k_sin_mul
        )
    ...
    def get_position_ids(self, qk_matmul_node: NodeProto) -> List[NodeProto]:
        ...
        input_position_ids_not_implemented = NotImplementedError(
            "Adding position ids through inputs is currently not supported."
        )
        ...
        _, q_or_k_parent = get_parent(
            qk_matmul_node, self.mha2sha_optim.get_node_by_output_name
        )
        ...
        if (
            set(position_keys)
            <= self.mha2sha_optim.mha_model_input_names_index_dict.keys()
        ):
        ...
        if set(position_keys) <= self.mha2sha_optim.get_node_by_input_name.keys():
            position_ids = [
                self.mha2sha_optim.get_node_by_input_name[k] for k in position_keys
            ]
                ...
                filter(
                    ...
                    [
                        get_next_node_up_based_on_cond(
                            ...
                            == "MatMul",  # End search if we hit a MatMul (Q or K)
                        )
                        for trig in ("cos", "sin")
                    ],
                )
        ...
        if not position_ids:
            log_debug(
                "No position ids with names cos and sin found. Searching for rotary names instead."
            )
            ...
            position_ids = [
                get_next_node_up_based_on_cond(
                    q_or_k_parent,
                    self.mha2sha_optim.get_node_by_output_name,
                    node_found_cond=lambda node: rotary in node.name
                    and node.op_type == "Unsqueeze",
                    node_end_search_cond=lambda node: node.op_type
                    == "MatMul",  # End search if we hit a MatMul (Q or K)
                )
        ...
        assert (
            len(position_ids) == 2
        ), f"Expected only two position ids, Got -> {len(position_ids)}"
        position_id_types = [isinstance(p, NodeProto) for p in position_ids]
        assert all(
            position_id_types
        ), f"Expected position ids to be NodeProto. Got -> {position_id_types=}"
        return position_ids
    ...
    def get_ropes_gather_output(self, rope_trig_node: NodeProto) -> str:
        ...
        def is_gather(node: NodeProto):
            nodes_children = get_children(
                node, self.mha2sha_optim.get_node_by_input_name
            )
            return (
                node.op_type == "Gather"
                and len(nodes_children) == 1
                and nodes_children[0].op_type == "Unsqueeze"
            )

        node = get_next_node_down_based_on_cond(
            rope_trig_node,
            self.mha2sha_optim.get_node_by_input_name,
            node_found_cond=is_gather,
        )
        assert (
            len(node.output) == 1
        ), f"Expected to find 1 output. Instead got {node.output=}"
    ...
    def get_mha_rope_nodes(self, linear_node, qk_matmul_node) -> Optional[RopeNode]:
        ...
        try:
            ...
            temp_Mul_rope_cos_1, temp_Mul_rope_sin_1 = (
                self.mha2sha_optim.get_node_by_input_name[slice_1.output[0]]
            )
            if (
                "cos" in temp_Mul_rope_cos_1.input[0]
                or "cos" in temp_Mul_rope_cos_1.input[1]
            ):
                Mul_rope_cos_1, Mul_rope_sin_1 = (
                    temp_Mul_rope_cos_1,
                    temp_Mul_rope_sin_1,
                )
            elif (
                "cos" in temp_Mul_rope_sin_1.input[0]
                or "cos" in temp_Mul_rope_sin_1.input[1]
            ):
                Mul_rope_sin_1, Mul_rope_cos_1 = (
                    temp_Mul_rope_cos_1,
                    temp_Mul_rope_sin_1,
                )
            else:
                raise ValueError(
                    f"Except cos/sin input to 1st slice Rope Mul, but get {temp_Mul_rope_sin_1.input}, {temp_Mul_rope_cos_1.input}"
                )

            # get Mul_rope_cos_2 Mul_rope_sin_2
            temp_Mul_rope_cos_2, temp_Mul_rope_sin_2 = (
                self.mha2sha_optim.get_node_by_input_name[slice_2.output[0]]
            )
            if (
                "cos" in temp_Mul_rope_cos_2.input[0]
                or "cos" in temp_Mul_rope_cos_2.input[1]
            ):
                Mul_rope_cos_2, Mul_rope_sin_2 = (
                    temp_Mul_rope_cos_2,
                    temp_Mul_rope_sin_2,
                )
            elif (
                "cos" in temp_Mul_rope_sin_2.input[0]
                or "cos" in temp_Mul_rope_sin_2.input[1]
            ):
                Mul_rope_sin_2, Mul_rope_cos_2 = (
                    temp_Mul_rope_cos_2,
                    temp_Mul_rope_sin_2,
                )
            else:
                raise ValueError(
                    f"Except cos/sin input to 2nd slice Rope Mul, but get {temp_Mul_rope_cos_2.input}, {temp_Mul_rope_sin_2.input}"
                )

            rope_sub = self.mha2sha_optim.get_node_by_input_name[
                Mul_rope_cos_1.output[0]
            ][0]
            rope_add = self.mha2sha_optim.get_node_by_input_name[
                Mul_rope_cos_2.output[0]
            ][0]

            rope_concat = self.mha2sha_optim.get_node_by_input_name[rope_sub.output[0]][
                0
            ]
            ...
            assert (
                rope_sub
                == self.mha2sha_optim.get_node_by_input_name[Mul_rope_sin_2.output[0]][
                    0
                ]
            ), "Failed to get rope_sub"
            assert rope_sub.op_type == "Sub", f"rope_sub got op type {rope_sub.op_type}"
            assert (
                rope_add
                == self.mha2sha_optim.get_node_by_input_name[Mul_rope_sin_1.output[0]][
                    0
                ]
            ), "Failed to get rope_add"
            assert rope_add.op_type == "Add", f"rope_add got op type {rope_add.op_type}"
            assert (
                rope_concat
                == self.mha2sha_optim.get_node_by_input_name[rope_add.output[0]][0]
            ), "Failed to get rope_concat"
            assert (
                rope_concat.op_type == "Concat"
            ), f"rope_concat got op type {rope_concat.op_type}"
            assert end_concat == rope_concat, "Rope end node is not rope concat"
            assert (
                rope_concat.input[0] == rope_sub.output[0]
            ), f"Rope concat input 1 is not rope_sub, got {rope_concat.input[0]}"
            assert (
                rope_concat.input[1] == rope_add.output[0]
            ), f"Rope concat input 2 is not rope_add, got {rope_concat.input[1]}"
            ...
            log_warning(
                f'Failed to verify MHA ROPE patten with expected pattern with exception "{e}". Will NOT map encodings in ROPE for {linear_node.name}.'
            )
            if self.mha2sha_optim.strict:
                log_warning(
                    "Fail script MHA2SHA conversion because MHA in ROPE doesn't match the golden ROPE we support.\
