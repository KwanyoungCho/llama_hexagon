...
class Config(object):
    def __init__(self, cfg_file, cfg_from_json, outputbasedir, devicelist, hostname, deviceostype,
        ...
        elif deviceostype == self._product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64 or deviceostype == self._product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X:
            self._architectures = [self._product.ARCH_AARCH64]
                        ...
                        if "hexagon" in _compiler and ("DSP" in self.runtimes or "DSP_FP16" in self.runtimes):
                            if self._dsp_type is None:
                                ...
                                if _compiler == device_type:
                                    if self.device_os_type in [self._product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64,self._product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X]:
    ...
    def get_device_artifacts_bin(self, runtime, windows_bin_toolchain=None):
        return '/'.join([self.__device_artifacts_helper(runtime,windows_bin_toolchain=windows_bin_toolchain), 'bin'])

    def get_device_artifacts_lib(self, runtime,toolchain=None):
        return '/'.join([self.__device_artifacts_helper(runtime,toolchain=toolchain), 'lib'])

    def get_exe_name(self):
        return self._product.BATCHRUN_EXE

    def __device_artifacts_helper(self, runtime,toolchain=None,windows_bin_toolchain=None):
        '''
        ...
        '''
        if self.device_os_type and (self.device_os_type == self._product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64 or self.device_os_type == self._product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X):
            anchor = self._product.RUNTIME_LIB_WIN
        else:
            anchor = self._product.RUNTIME_LIB
        if(toolchain and 'windows' in toolchain and sys.platform.startswith('win')):
            return '/'.join([
                    self.device_path, self._product.ARTIFACT_DIR, toolchain])
        
        if(windows_bin_toolchain and 'windows' in windows_bin_toolchain and sys.platform.startswith('win')):
            return '/'.join([
                    self.device_path, self._product.ARTIFACT_DIR, windows_bin_toolchain])
