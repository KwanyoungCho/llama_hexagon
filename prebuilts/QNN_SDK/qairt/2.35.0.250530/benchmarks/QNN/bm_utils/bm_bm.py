...
class BenchmarkFactory(object):
    ...
    def make_benchmarks(config, product, device_os_type, htp_serialized, shared_buffer,
                        toolchain=None,windows_bin_toolchain=None):
        ...
        for runtime, flavor in config.return_valid_run_flavors():
            for measurement in config.measurements:
                dev_bin_path = config.get_device_artifacts_bin(runtime,windows_bin_toolchain=windows_bin_toolchain)
                dev_lib_path = config.get_device_artifacts_lib(runtime,toolchain=toolchain)
...
class SnapDnnCppDroidBenchmark(object):
    ...
    def __create_diagview_script(self):
        ...
        cmds.append(run_cmd)
        cmd_script_path = ''
        if self.device_os_type not in  [self.product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64,self.product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X]:
        ...
        with open(cmd_script_path, 'w') as cmd_script:
            for cmd in cmds:
                if self.device_os_type in  [self.product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64,self.product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X]:
                    cmd = cmd.replace('/', '\\')
    ...
    def __create_script(self, dsp_type, arm_prepare, enable_cache, cdsp_id,netrun_options,toolchain=None,
                        windows_bin_toolchain=None):
        if self.device_os_type not in [self.product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64,self.product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X]:
            cmds = [
            ...
            ADSP_PATH= 'export ADSP_LIBRARY_PATH=\"' + self._dep_lib_dir +  '/../../dsp/lib;/system/lib/rfsa/adsp;'+ \
                '/usr/lib/rfsa/adsp;/vendor/dsp/cdsp;/system/vendor/lib/rfsa/adsp;/dsp;/etc/images/dsp;'
            if cdsp_id=='1':
                ADSP_PATH+='/usr/lib/dsp/cdsp;/usr/lib/dsp/cdsp1;\"'
            else:
                ADSP_PATH+='\"'
            cmds.append(ADSP_PATH)
            cmds.append('cd ' + self._model_dir)
            cmds.append('rm -rf ' + self.output_dir)
            cmds.append('chmod +x {}/*'.format(self._exe_dir))
        else:
            if(not toolchain and self.device_os_type in [self.product.CONFIG_DEVICEOSTYPES_WINDOWS_AARCH64,self.product.CONFIG_DEVICEOSTYPES_WINDOWS_ARM64X]):
                toolchain="aarch64-windows-msvc"
            cmds = ['set PATH=%PATH%;'+ self._dep_lib_dir +f"\..\..\{toolchain}\lib" + ';' + self._model_dir + \
                    ';' + os.path.abspath(self._dep_lib_dir + f'\..\..\hexagon-{dsp_type}\lib'),
        ...
        if 'DSP' in self.runtime:
            if dsp_type in ['v68', 'v69', 'v69-plus', 'v73', 'v75', 'v79', 'v81']:
    ...
    def pre_commands(self, dsp_type, arm_prepare, enable_cache, cdsp_id,netrun_options,toolchain=None,
                     windows_bin_toolchain=None):
        ...
        cmd_script = self.__create_script(dsp_type, arm_prepare, enable_cache, cdsp_id,netrun_options,toolchain=toolchain,
                                          windows_bin_toolchain=windows_bin_toolchain)
