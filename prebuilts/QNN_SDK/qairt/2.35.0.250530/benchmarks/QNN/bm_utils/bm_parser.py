...
class ArgsParser(object):
    def __init__(self, program_name, args_list):
        ...
        optional.add_argument('--dsp_type', help='Specify DSP variant for QNN BM run',
                              required=False,
                              choices=['v65', 'v66', 'v68', 'v69', 'v69-plus', 'v73', 'v75', 'v79', 'v81'], default=None)
        ...
        optional.add_argument('--netrun_options', help="Specify all extra options to qnn-net-run seperated by , in ''  ex: '--use_mmap,perf_profile burst' or '--use_mmap ' (give space if it is single perameter) ",
                               required=False, default=None,type=str)
        optional.add_argument('--windows_bin_toolchain', required=False, default=None, action="store",
                          help="Toolchain Type. Options: ['aarch64-windows-msvc','x86_64-windows-msvc'] ")
        optional.add_argument('--driver_path', required=False,default=None, action="store",
                          help=" path to htp driver with signed cat file")
    ...
    @property
    def netrun_options(self):
        return self._args['netrun_options']
    ...
    @property
    def windows_bin_toolchain(self):
        return self._args['windows_bin_toolchain']

    @property
    def driver_path(self):
        return self._args['driver_path']
    ...
    def parse(self, input_dir):
        ...
        while line < len(diag_log_output):
            ...
            if statistic_key in self.MAJOR_STATISTICS_1:
                stat_line = line + 2
                if statistic_key in List_of_sources:
                    line = stat_line
                    continue
                List_of_sources.append(statistic_key)
