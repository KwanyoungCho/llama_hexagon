...
void showHelp() {
  std::cout
      ...
      << "                                  If there are multiple graphs in the bin, this has\n"
      << "                                  to be comma separated list of input list files.\n"
      << "                                  If only certain graphs are enabled, each input list\n"
      << "                                  should correlate to the respective graph specified\n"
      << "                                  by --enabled_graphs\n"
      << "\n"
      ...
      << "\n"
      << "  --enabled_graphs    <VAL>       Comma separated list of names of graphs to enable.\n"
      << "                                  If --mem_limit_hint is also set and nonzero, the\n"
      << "                                  first graph name will be the first graph loaded in.\n"
      << "\n"
      << "  --mem_limit_hint    <VAL>       Memory limit hint used for graph switching. Set to\n"
      << "                                  0 to load all graphs.\n"
      << "\n"
      << "  --execution_order   <VAL>       Comma separated list of graph names to execute. Must\n"
      << "                                  be used in conjunction with a non-zero value for.\n"
      << "                                  --mem_limit_hint to demonstrate graph switching.\n"
      << "\n"
      << "  --random_inputs                 Generates random values for the input tensors\n"
      << "                                  --random_inputs and --input_list are mutually\n"
      << "                                  exclusive.\n"
      << "\n"
...
std::unique_ptr<sample_app::QnnSampleApp> processCommandLine(int argc, char** argv) {
  enum OPTIONS {
    ...
    OPT_ENABLE_GRAPHS    = 16,
    OPT_MEM_LIMIT_HINT   = 17,
    OPT_RANDOM_INPUTS    = 18,
    OPT_EXECUTION_ORDER  = 19
  };
  ...
  static struct pal::Option s_longOptions[] = {
      ...
      {"enabled_graphs", pal::required_argument, NULL, OPT_ENABLE_GRAPHS},
      {"mem_limit_hint", pal::required_argument, NULL, OPT_MEM_LIMIT_HINT},
      {"execution_order", pal::required_argument, NULL, OPT_EXECUTION_ORDER},
      {"random_inputs", pal::no_argument, NULL, OPT_RANDOM_INPUTS},
  ...
  while ((opt = pal::getOptLongOnly(argc, argv, "", s_longOptions, &longIndex)) != -1) {
    switch (opt) {
      ...
      case OPT_ENABLE_GRAPHS:
        enabledGraphs = pal::g_optArg;
        if (enabledGraphs.empty()) {
          showHelpAndExit("There must be at least one enabled graph.\n");
        }
        break;

      case OPT_MEM_LIMIT_HINT:
        memLimitHint = static_cast<uint64_t>(sample_app::parseUintArg(pal::g_optArg));
        break;

      case OPT_EXECUTION_ORDER:
        executionOrder = pal::g_optArg;
        if (executionOrder.empty()) {
          showHelpAndExit("There must be at least one graph to execute.\n");
        }
        break;

      case OPT_RANDOM_INPUTS:
        randomInputs = true;
        break;
  ...
  if (!inputListPaths.empty() && randomInputs) {
    showHelpAndExit("Error: both --input_list and --random_inputs specified");
  }
  ...
  if (!randomInputs && inputListPaths.empty()) {
