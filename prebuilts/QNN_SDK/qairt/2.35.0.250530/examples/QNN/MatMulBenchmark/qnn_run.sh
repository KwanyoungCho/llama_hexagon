...
# INT16 combinations
echo "=== INT16 Tests ==="
adb shell "cd /data/local/tmp/chokwans99/QNN_benchmark/ && export LD_LIBRARY_PATH=/data/local/tmp/chokwans99/QNN_benchmark/ && ./MatMulBenchmark -a -t int16 -qa scale -qb scale -qc scale -i 30 -maxseq 256" > ./matmul_results/int16_int16.txt

# INT8 combinations
echo "=== INT8 Tests ==="
# adb shell "cd /data/local/tmp/chokwans99/QNN_benchmark/ && export LD_LIBRARY_PATH=/data/local/tmp/chokwans99/QNN_benchmark/ && ./MatMulBenchmark -a -t int8 -qa scale -qb scale -qc scale -i 30 -maxseq 1024" > ./matmul_results/int8_int8.txt
