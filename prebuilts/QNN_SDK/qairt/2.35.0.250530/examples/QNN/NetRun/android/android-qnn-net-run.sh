...
#
set -e
BACKEND_LIST="cpu;gpu;htp-v68;htp-v69;htp-v73;dsp-v66;dsp-v65;saver;htp-v75;htp-v79"
...
HEXAGON_V79_SKEL_PATH=${QNN_SDK_ROOT}/lib/hexagon-v79/unsigned/libQnnHtpV79Skel.so
...
case "$backend" in
  ...
  htp-v73 )
    ...
    ADSP_LIBRARY_PATH="export ADSP_LIBRARY_PATH=\"${TARGET_ROOT}\""
    LD_LIBRARY_PATH="export LD_LIBRARY_PATH=\"${TARGET_ROOT}\""
    QNN_NET_RUN_CMD="${ADSP_LIBRARY_PATH} && ${LD_LIBRARY_PATH} && ""./qnn-net-run --model lib"${MODEL}".so --input_list input_list_float.txt --backend libQnnHtp.so"
    ;;
  htp-v75 )
    adb push ${QNN_LIB_ROOT}/libQnnHtp.so ${TARGET_ROOT}
  ...
  htp-v79 )
    adb push ${QNN_LIB_ROOT}/libQnnHtp.so ${TARGET_ROOT}
    adb wait-for-device push ${QNN_LIB_ROOT}/libQnnHtpPrepare.so ${TARGET_ROOT}
    adb wait-for-device push ${QNN_LIB_ROOT}/libQnnHtpV79Stub.so ${TARGET_ROOT}
    adb wait-for-device push ${HEXAGON_V79_SKEL_PATH} ${TARGET_ROOT}
    ADSP_LIBRARY_PATH="export ADSP_LIBRARY_PATH=\"${TARGET_ROOT}\""
    LD_LIBRARY_PATH="export LD_LIBRARY_PATH=\"${TARGET_ROOT}\""
    QNN_NET_RUN_CMD="${ADSP_LIBRARY_PATH} && ${LD_LIBRARY_PATH} && ""./qnn-net-run --model lib"${MODEL}".so --input_list input_list_float.txt --backend libQnnHtp.so"
    ;;
